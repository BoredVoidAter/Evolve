...
{
    public enum CreatureType { Biped, Spider, Centipede, Starfish, TentacleAlien }
    ...
    private BodyPlanDNA GetBipedDNA()
    {
        return new BodyPlanDNA {
            ...
            Limbs = new List<LimbDNA> {
                // Legs
                new LimbDNA { Type = LimbType.Leg, AttachedSegmentIndex = 0, JointCount = 3, BoneLengths = new float[] { 0.8f, 0.8f, 0.2f }, Pitch = 90, Yaw = -20, Roll = 0, AttachmentSpacing = 1.2f },
                // Manipulators (Arms) - Fixed lengths and pitched slightly forward so they look like swinging arms instead of dragging legs
                new LimbDNA { Type = LimbType.Manipulator, AttachedSegmentIndex = 2, JointCount = 3, BoneLengths = new float[] { 0.6f, 0.6f, 0.2f }, Pitch = 45, Yaw = 0, Roll = 0, AttachmentSpacing = 0.8f }
            }
        };
    }
    ...
    {
        return new BodyPlanDNA {
            Symmetry = SymmetryType.Radial, RadialCount = 6, SpineSegments = 2, SpineSegmentLength = 0.5f,
            Limbs = new List<LimbDNA> {
                // Wiggling Tentacles for locomotion
