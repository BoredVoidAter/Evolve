...
public class HexGridVisualizer : MonoBehaviour
{
    ...
    private void RenderRiversAsMesh(HexGridData data)
    {
        ...
        foreach (var kvp in data.cells)
        {
            ...
            // Determine which edges have an active river connection
            ...
            // Check if the river goes straight across the hex
            bool isStraightRiver = activeArms.Count == 2 && (activeArms[0] + 3) % 6 == activeArms[1];
            
            if (isStraightRiver)
            {
                // Bypass hub rendering entirely to just draw straight quads seamlessly connected
            }
            else if (activeArms.Count <= 1)
            {
                // If it's a dead end (source), render the full center hub
                for (int i = 0; i < 6; i++) hubTrianglesToRender.Add(i);
            }
            else
            {
                // Otherwise only render the shortest continuous path of pie slices
                // to bridge between the active arms smoothly, avoiding the "knot" bulge
                ...
                // Find the largest angular gap between river arms
                ...
                // Connect the arms using everything EXCLUDING the max gap
                int currT = maxGapEnd;
                while (true)
                {
            ...
            for (int i = 0; i < 6; i++)
            {
                if (cell.riverEdges[i] != 0)
                {
                    ...
                    if (isStraightRiver)
                    {
                        // To avoid any curve, extrude the points perfectly parallel to the edge
